...
public class TestInternalHttpFilterPlugin {
  ...
  @Rule public MockServerRule mockServerRule = new MockServerRule(this, 18888);
  ...
  public void testThrowExceptionAbsentUrlWithNonSampleDataMode() {
    ConfigSource config =
        configFromYamlString(
            "type: internal_http", "columns:", "  - {name: col1, type: timestamp}");
    final Schema inputSchema = schema();
    assertThrows(
        ConfigException.class,
        () -> {
          internalHttpFilterPlugin.transaction(
              config,
              inputSchema,
              new Control() {
                @Override
  ...
  public void testThrowExceptionAbsentColumnsWithNonSampleDataMode() {
    ConfigSource config =
        configFromYamlString("type: internal_http", "url: http://foo.bar.com:18888");
    final Schema inputSchema = schema();
    assertThrows(
        ConfigException.class,
        () -> {
  ...
  // Filter tests
  @SuppressWarnings(
      "deprecation") // For the use of org.embulk.spi.time.Timestamp and org.embulk.spi.util.Pages.
  @Test
  public void testSampleDataModeOutput() {
    ConfigSource config = configFromYamlString("type: internal_http", "sample_data_mode: true");
    ...
    internalHttpFilterPlugin.transaction(
        ...
        new Control() {
          ...
          public void run(TaskSource taskSource, Schema outputSchema) {
            ...
            try (PageOutput pageOutput =
                internalHttpFilterPlugin.open(
                    taskSource, inputSchema, outputSchema, mockPageOutput)) {
              for (Page page :
                  PageTestUtils.buildPage(
            ...
            List<Object[]> records =
                org.embulk.spi.util.Pages.toObjects(outputSchema, mockPageOutput.pages);
    ...
    new MockServerClient("localhost", 18888)
        .when(
            HttpRequest.request()
                ...
                .withPath("/")
                .withBody(
                    "{\"rows\":[{\"long_col\":1,\"double_col\":2.0,\"boolean_col\":false,\"string_col\":\"foo\",\"timestamp_col\":\"1970-01-01 00:00:04 +0000\",\"json_col\":{\"foo\":\"bar\"}}]}"))
        .respond(
            HttpResponse.response()
                .withStatusCode(200)
                .withBody(
                    "{\"rows\":[{\"long_col\":2,\"double_col\":3.0,\"boolean_col\":true,\"string_col\":\"bar\",\"timestamp_col\":\"1970-01-01 00:00:05 +0000\",\"json_col\":{\"bar\":\"baz\"}}]}"));
    ConfigSource config =
        configFromYamlString(
            "type: internal_http",
    ...
    internalHttpFilterPlugin.transaction(
        ...
        new Control() {
          ...
          public void run(TaskSource taskSource, Schema outputSchema) {
            ...
            assertEquals(
                newMapBuilder().put(newString("bar"), newString("baz")).build(), record[5]);
          }
        });
  }
  ...
  public void testNonSampleModeWithNullRowOutput() {
    new MockServerClient("localhost", 18888)
        ...
        .respond(HttpResponse.response().withStatusCode(200).withBody("{\"rows\":[null]}"));
    ConfigSource config =
        configFromYamlString(
            "type: internal_http",
            "url: http://localhost:18888",
            "columns:",
            "  - {name: long_col, type: long}");
    final Schema inputSchema = schema(new Column(0, "long_col", LONG));
    internalHttpFilterPlugin.transaction(
        config,
        inputSchema,
        new Control() {
          ...
          public void run(TaskSource taskSource, Schema outputSchema) {
            ...
            try (PageOutput pageOutput =
                internalHttpFilterPlugin.open(
                    taskSource, inputSchema, outputSchema, mockPageOutput)) {
              for (Page page :
                  PageTestUtils.buildPage(runtime.getBufferAllocator(), inputSchema, 1L)) {
                pageOutput.add(page);
              }
              pageOutput.finish();
            }
    ...
    new MockServerClient("localhost", 18888)
        ...
        .respond(
            HttpResponse.response()
                .withStatusCode(200)
                .withBody(
                    "{\"rows\":[{\"long_col\":null,\"double_col\":null,\"boolean_col\":null,\"string_col\":null,\"timestamp_col\":null,\"json_col\":null}]}"));
    ConfigSource config =
        configFromYamlString(
            "type: internal_http",
            "url: http://localhost:18888",
  ...
  public void testNonSampleModeWithNonDefaultFormatTsOutput() {
    ...
    final Schema inputSchema = schema(new Column(0, "timestamp_col", TIMESTAMP));
    internalHttpFilterPlugin.transaction(
        config,
        inputSchema,
        new Control() {
