...
public class InternalHttpFilterPlugin implements FilterPlugin {
  ...
  public void transaction(ConfigSource config, Schema inputSchema, FilterPlugin.Control control) {
    ...
    validateConfig(task);
    Schema outputSchema =
        task.getSampleDataMode() ? buildSampleDataOutputSchema() : buildDefaultOutputSchema(task);
    ...
    for (ColumnConfig column : columns) {
      if (column.getType().getName().equals("timestamp")) {
        TimestampFormatter formatter =
            TimestampFormatter.builder(column.getFormat(), true)
  ...
  // For the use of org.embulk.spi.time.Timestamp, pageReader.getTimestamp, pageReader.getJson,
  // pageBuilder.setJson and JsonParser
  @SuppressWarnings("deprecation")
  @Override
  public PageOutput open(
      TaskSource taskSource, Schema inputSchema, Schema outputSchema, PageOutput output) {
      ...
      public void add(Page page) {
        ...
        while (pageReader.nextRecord()) {
          ...
          for (Column column : inputSchema.getColumns()) {
            ...
            if (Types.STRING.equals(type)) {
              ...
              DateTimeFormatter formatter =
                  DateTimeFormatter.ofPattern(defaultJavaTimestampFormat).withZone(ZoneOffset.UTC);
              requestPageNode.put(
                  column.getName(), formatter.format(pageReader.getTimestamp(column).getInstant()));
            } else if (Types.JSON.equals(type)) {
              try {
                requestPageNode.set(
                    column.getName(), mapper.readTree(pageReader.getJson(column).toString()));
              } catch (IOException e) {
          ...
          if (task.getSampleDataMode()) {
            try {
              pageBuilder.setJson(
                  new Column(0, sampleDataModeColumnName0, Types.JSON),
                  new org.embulk.util.json.JsonParser()
                      .parse(mapper.writeValueAsString(requestPageNode)));
              pageBuilder.setJson(
                  new Column(1, sampleDataModeColumnName1, Types.JSON),
                  new org.embulk.util.json.JsonParser()
                      .parse(mapper.writeValueAsString(schemaNode)));
        ...
        try {
          ...
          RequestConfig config =
              RequestConfig.custom().setSocketTimeout(httpRequestTimeoutSec * 1000).build();
          try (CloseableHttpClient httpClient =
            ...
            try (CloseableHttpResponse httpResponse = httpClient.execute(httpPost)) {
              if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                Iterator<JsonNode> responseRootIterator =
                    responseRootNode.get(requestJsonRootAttributeName).elements();
                  ...
                  for (Column column : outputSchema.getColumns()) {
                    ...
                    if (Types.STRING.equals(type)) {
                      ...
                      pageBuilder.setTimestamp(
                          column,
                          org.embulk.spi.time.Timestamp.ofInstant(
                              timestampFormatterMap.get(column.getName()).parse(val.asText())));
                    } else if (Types.JSON.equals(type)) {
                      pageBuilder.setJson(
                          column, new org.embulk.util.json.JsonParser().parse(val.toString()));
                    }
                  }
                  pageBuilder.addRecord();
                }
